...
# Matches both old format (no worker_id) and new format (with worker= and id=/worker_id=)
FAIL_RE = re.compile(
    r"n=\s*(\d+)\s*\|.*?FAIL.*?\|\s*steps=\s*(\d+)\s*\|\s*peak=(\d+)"
    r"(?:.*?\|\s*worker=([^|]+))?"
    r"(?:.*?\|(?:\s*id=|\s*worker_id=)([^\s|]+))?"
)


def parse_args():
    p = argparse.ArgumentParser(description="Merge Collatz FAIL result files")
...
def parse_file(path):
    """Parse a FAIL results file, returning a list of record dicts."""
    records = []
    try:
        with open(path) as f:
            for line in f:
                ...
                if m:
                    records.append({
                        ...
                        "steps":     int(m.group(2)),
                        "peak":      int(m.group(3)),
                        "worker":    (m.group(4) or "").strip(),
                        "worker_id": (m.group(5) or "").strip(),
                        "raw":       line.rstrip(),
                    })
